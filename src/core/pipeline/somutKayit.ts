...
export interface SomutKayit {
  kimlik: string;
  etiket: string;
}

export interface SomutSonuc {
  tablo: string;
  kimlikKolonu: string;
  kayitlar: SomutKayit[];
...
function tabloBul(olcumSql: string, tablolar: Tablo[]): Tablo | null {
...
function etiketKolonu(tablo: Tablo, kimlik: string): string | null {
...
function baskinGrup(sonuc: OlcumSonucu, teshis: Teshis): { kolon: string; deger: string } | null {
  const bulgu = teshis.bulgular.find((b) => b.etiket && (b.tur === "yigilma" || b.tur === "aykiri"));
 ...
 */
export async function somutKayitlariGetir(
  sonuc: OlcumSonucu, teshis: Teshis, azami = 5
): Promise<SomutSonuc | null> {
  if (!sonuc.sql) return null;

  const tablolar = await semaGetir();
  const tablo = tabloBul(sonuc.sql, tablolar);
  if (!tablo) return null;

  const kimlik = kimlikKolonu(tablo);
  if (!kimlik) return null;

  const etiket = etiketKolonu(tablo, kimlik);
  const grup = baskinGrup(sonuc, teshis);
  ...
  try {
    ...
    const kayitlar: SomutKayit[] = (y.recordset as Record<string, unknown>[]).map((r) => ({
    ...
    const atananKolon = tablo.kolonlar.find((c) => /^AtananKisi$/i.test(c.ad));
    if (atananKolon) {
      try {
...
export function somutKayitMetni(s: SomutSonuc): string {
