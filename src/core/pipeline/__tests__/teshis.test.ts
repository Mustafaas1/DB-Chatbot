...
const turler = (s: ReturnType<typeof diagnose>) => s.findings.map((b) => b.tur);

describe("yigilma", () => {
  it("tek grup baskin oldugunda yakalanir", () => {
    // 4900 / toplam 4959 = %98
    const t = diagnose(sonuc(["Asama", "Adet"],
      [["Tamamlandı", 4900], ["Beklemede", 47], ["İşlemde", 12]]));
    expect(turler(t)).toContain("yigilma");
    expect(t.findings[0]?.etiket).toBe("Tamamlandı");
    expect(t.findings[0]?.oran).toBeGreaterThan(0.9);
    expect(t.toplam).toBe(4959);
  });

  it("esigin altinda yigilma denmez", () => {
    const t = diagnose(sonuc(["G", "A"], [["a", 50], ["b", 50]]));
